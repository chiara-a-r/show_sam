...
def write_course_table():
	...
	create_table_COURSES = "CREATE TABLE IF NOT EXISTS COURSE (id Integer PRIMARY KEY, courseCode text, courseTime text)"
	cursorObj.execute(create_table_COURSES)
	conn.commit()
	with open('courses.csv','r') as csv_in:
		reader = csv.reader(csv_in)
		for row in reader:
			sql = "INSERT INTO COURSE(courseCode,courseTime) VALUES('%s','%s')" %(row[0],row[1])
